      ...
      if (results.length > 0) {
        ...
        bcrypt
          ...
          .then((cryptedPassword) => {
            ...
            db.query(
              `INSERT INTO user_groupomania.user (nom, prenom, email, password ) VALUES ('${req.body.nom}','${req.body.prenom}','${req.body.email}','${cryptedPassword}')`,
        ...
        bcrypt.compare(req.body.password, results[0].password).then((valid) => {
          ...
          if (!valid) {
            ...
            res.status(200).json({
              ...
              token: jwt.sign({ userId: results[0].id }, 'my_secret_key', {
                expiresIn: '8h',
              }),
            });
          }
...
exports.getUserProfile = (req, res, next) => {
  ...
  db.query(
    'SELECT * FROM user_groupomania.user WHERE id=?',
    [req.params.id],
    (error, result, rows) => {
...
exports.deleteUserProfile = (req, res, next) => {
  ...
  db.query(
    'DELETE FROM user_groupomania.user WHERE id=?',
    [req.params.id],
    (error, result) => {
      if (error) {
        return res.status(400).json({ error });
